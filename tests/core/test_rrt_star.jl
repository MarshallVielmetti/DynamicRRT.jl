...
@testset "RRTStar" begin
    ...
    @testset "Path Extraction" begin
        problem = MockProblem(@SVector [3.0, 0.0])
        start_state = @SVector [0.0, 0.0]
        hash_map_widths = @SVector [1.0, 1.0]
        solution = RRTStar.setup(problem, start_state, hash_map_widths)
        ...
        node_goal = RRTStar.ChildNode(SVector{2,Float64}(3.0, 0.0), RRTStar.LinearIndex(i_b), 3.0)
        ...
        @testset "best_path!" begin
            # Tests that the best_path! function correctly identifies the lowest-cost path to the goal.
